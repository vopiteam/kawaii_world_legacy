...
void TouchScreenGUI::init(ISimpleTextureSource *tsrc)
{
	...
	if (m_fixed_joystick) {
		...
		m_joystick_btn_off = initJoystickButton(joystick_off_id,
				rect<s32>(button_size * 1.5,
						  m_screensize.Y - button_size * 3,
						  button_size * 3,
						  m_screensize.Y - button_size * 1.5), 0);
	}
	...
	initButton(inventory_id,
			   rect<s32>(m_screensize.X - button_size * 0.8,
						 (m_screensize.Y * 0.5) - ((0.01 * button_size) + button_size * 0.8),
						 m_screensize.X,
						 (m_screensize.Y * 0.5) - (0.01 * button_size)),
			   L"inv", false);

	// init crunch button
	initButton(crunch_id,
			   rect<s32>(m_screensize.X - button_size * 2.75,
						 m_screensize.Y - button_size * 1.1,
						 m_screensize.X - button_size * 1.75,
						 m_screensize.Y - button_size * 0.1),
			   L"H", false);
	...
	initButton(fast_id,
			   rect<s32>(m_screensize.X - button_size * 1.5,
						 m_screensize.Y - button_size * 1.65,
						 m_screensize.X - button_size * 0.5,
						 m_screensize.Y - button_size * 0.65),
			   L"fastmove", false);
	...
	s32 button_sz = button_size * 0.8;
	s32 half_button = button_sz * 0.5;
	s32 button_z5 = 0.03 * button_sz;
	s32 screen_center_x = m_screensize.X * 0.5;

	// init pause button [1]
	initButton(escape_id,
			   rect<s32>(screen_center_x - half_button - 2 * button_z5 - 2 * button_sz,
						 0,
						 screen_center_x - half_button - 2 * button_z5 - button_sz,
						 button_sz),
			   L"Exit", false);

	// init camera button [2]
	initButton(camera_id,
			   rect<s32>(screen_center_x - half_button - button_z5 - button_sz,
						 0,
						 screen_center_x - half_button - button_z5,
						 button_sz),
			   L"camera", false);

	// init rangeselect button [3]
	initButton(range_id,
			   rect<s32>(screen_center_x - half_button,
						 0,
						 screen_center_x + half_button,
						 button_sz),
			   L"rangeview", false);

	// init minimap button [4]
	initButton(minimap_id,
			   rect<s32>(screen_center_x + half_button + button_z5,
						 0,
						 screen_center_x + half_button + button_z5 + button_sz,
						 button_sz),
			   L"minimap", false);
	...
	// init chat button
	initButton(chat_id,
			   rect<s32>(screen_center_x + half_button + 2 * button_z5 + button_sz,
						 0,
						 screen_center_x + half_button + 2 * button_z5 + 2 * button_sz,
						 button_sz),
			   L"Chat", false);
